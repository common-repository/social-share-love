<?php
/*
Plugin Name: Social Share Love
Version: 1.0
Plugin URI: -
Author: noucktourno
Author URI: -
Description: Social Share Love - Share your website articles on the following social networks: yahoo, google, facebook, twitter, technorati, stumbleupon, delicious, digg, friendfeed, myster-wong, myspace, reddit, slashdot.

    Copyright 2011 noucktourno (Email: socialsharelove@gmail.com)
...
class social_bookmarks
{
	...
	function social_bookmarks()
	{
		...
		$this->plugin_dir = get_option('siteurl').'/wp-content/plugins/social-share-love/'; 
		...
		$xml_sites = $this->read_xml(dirname(__FILE__)."/websites.xml");
		...
		// Set Default settings as current if no options in wordpress database
		// Otherwise populate current settings from wordpress database
	...
	{
		print("<link rel=\"stylesheet\" type=\"text/css\" media=\"screen\" href=\"".$this->plugin_dir."social_share_love.css\" />\n");
	}
	...
	function update_sites($data)
	{
		...
		// Compile option for wordpress database
	...
	function admin_menu()
	{
		...
		add_options_page('Social Share Love', 'Social Share Love', 8, __FILE__, array(&$this, 'plugin_options'));
...
//  		$this->debug_section();
	...
	function option_sites()
	{
		...
		print('<div class="wrap">');
		print('<h3>Social Networking Websites</h3>');
		print('<p>Select the social networks you want to display on your site:</p>');
	...
	function option_other()
	{
		...
		if($this->current_settings['dw_display'] == 1)
		{
			...
			$html .= "<option value=\"3\">in both single post and blog listing</option>\n";
		}
		...
		elseif($this->current_settings['dw_display'] == 3)
		{
			$html .= "<option value=\"1\">in the blog listing (index.php)</option>\n";
			$html .= "<option value=\"2\">when a single post is viewed</option>\n";
			$html .= "<option value=\"3\" selected>in both single post and blog listing</option>\n";
		}
		$html .= "</select>\n";
		$html .= ".\n";
		$html .= "</p>";
		...
		// Exclude these pages
		$html.= '<p>Do not display the social bookmarks links on the selected pages below:</p>';
